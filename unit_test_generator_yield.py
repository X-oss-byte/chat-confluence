...
def print_messages(messages, color_prefix_by_role=color_prefix_by_role, print_function=print) -> None:
    ...
    for message in messages:
        ...
        yield from print_function(f"{color_prefix}\n[{role}]\n{content}")
...
def print_message_delta(delta, color_prefix_by_role=color_prefix_by_role, print_function=print) -> None:
    ...
    if "role" in delta:
        ...
        yield from print_function(content, end="")
...
```""",
    ...
    if print_text:
        yield from print_messages(explain_messages, print_function=print_function)
    ...
    for chunk in explanation_response:
        ...
        if print_text:
            yield from print_message_delta(delta, print_function=print_function)
        if "content" in delta:
...
To help unit test the function above, list diverse scenarios that the function should be able to handle (and under each scenario, include a few examples as sub-bullets).""",
    ...
    if print_text:
        yield from print_messages([plan_user_message], print_function=print_function)
    ...
    if elaboration_needed:
        ...
        if print_text:
            yield from print_messages([elaboration_user_message], print_function=print_function)
...
```""",
    ...
    if print_text:
        yield from print_messages([execute_system_message, execute_user_message], print_function=print_function)
